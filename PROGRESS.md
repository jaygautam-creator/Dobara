...
— a ~33% relative margin, not huge. Published under its own "Break-even reporting"
heading in the README, stated plainly: below roughly the bottom half of the declared
range, `dobara` does not beat `razorpay_default`. Full account in `docs/DECISIONS.md`
[2026-08-26] (two entries). `make check` green throughout.
...
**Next action:** Phase 4 is substantively complete — headline (`dobara` beats
`razorpay_default` by ₹66/mandate, significant), the mechanism decomposition, the money
chart, and the break-even statement (both directions) are all built and published in
README, per `## CURRENT STATE` above. Genuinely outstanding, non-blocking: the other three
declared sensitivity axes (`ltv.horizon_cycles`, notification channel cost,
`date_change_offer.response_rate` incl. 0%) haven't been swept, only
`hazard_per_failure_notification`; and the `SBI`-specific restraint cost (abstention
*response*, not detection quality, is the next lever) is an open design question for a
future session. After that: Phase 5 (API + Razorpay test mode) or Phase 6 (frontend) —
user's call.
...
- [x] Sensitivity analysis on `hazard_per_failure_notification` across its full declared range [0.05, 0.15] (`eval/sensitivity.py`, `artifacts/sensitivity.json`) — the other three declared axes (LTV horizon, notification channel cost, response_rate incl. 0%) still not swept, genuinely outstanding

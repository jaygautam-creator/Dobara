...
**This is a held experiment, not a shipped decision.** `main` currently runs the
isotonic calibrator below (the numbers in "Honest metrics"); a full, measured
alternative configuration — Platt scaling on the recovery model, adopted per a
pre-registered rule, fully evaluated, then diagnosed rather than shipped — lives on
`experiment/platt-calibrator`. Both configurations are reported here in full. Nothing
is deleted, softened, or quietly chosen because it reads better.

**The result, stated directly:** we cut the argmax tie rate — the fraction of decisions
where the calibrator couldn't distinguish candidate dates, forcing
`agent/decide.py`'s restraint-first tie-break to pick instead — from **77.2% to
17.9%**, using a calibrator (Platt) that scored *better* on the pre-registered proxy
metric (Brier score, held-out, CI-overlap confirmed). And the system got
**significantly worse** on the metric that actually matters: net LTV moved from
**+₹65.71/mandate** to **−₹64.09/mandate [−₹81.50, −₹48.65]**, CI excluding zero, on 30
paired seeds. Both configurations fully measured. Both reported here.

**Ruled out as a bug, not just asserted.** Before treating this as a real finding, the
obvious alternative — a calibrator bug — was checked systematically: the recovery
model's underlying booster is bit-identical between the two configurations (only the
calibrator changed); Platt is strictly monotone and stays in-range; no unrelated code
change reaches the money path. What was found instead is a real, mechanically-explained
property — Platt, fit on an already-bounded probability rather than a raw margin, has a
hard mathematical ceiling (0.914 vs. isotonic's 1.0 on this population) and the agent
abstains meaningfully less under it (1,952.6 → 1,720.6 per seed, -11.9%) — not a
one-directional calibration bias in the operating band, which was specifically tested
for and not found. Full diagnostic, every check, every number:
`docs/DECISIONS.md` [2026-09-02] "Diagnosing the reversal: is it a bug, or a real
finding?".
...
project's own agent, in both directions, with both results kept. A project that only
ever reports the version of itself that looks good is not demonstrating restraint. This
README does both.
...
| Metric | Isotonic (`main`) | Platt (`experiment/platt-calibrator`) | Delta |
|---|---:|---:|---:|
...
**What ships is not decided by this README.** That's a call for whoever owns this
project to make with the full comparison in front of them, not something to bury inside
a favorable number.
...
**These specific numbers are from `experiment/platt-calibrator`, the held experiment
above — `main` currently runs isotonic; see that section for both.**)*

...
[2026-09-02] "The /evidence calibrator-experiment section"): this script cannot be
re-run to completion on a clean `main` checkout alone**, for the same reason
`scripts/measure_production_tie_rate.py` can't -- `main` ships isotonic only, and the
Platt joblib artifacts this script needs live solely on `experiment/platt-calibrator`.
The guard below fails loudly (not silently) if run where the installed model isn't the
expected Platt version, rather than producing a wrong comparison. The committed
`artifacts/date_shift_decomposition.json` on `main` is the pinned record of that
branch's measurement; re-verify it there (`git checkout experiment/platt-calibrator`),
not by re-running this script on `main`.
"""

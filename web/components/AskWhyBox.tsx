import type { AskWhyEntry } from "@/lib/types";
 ...
 *
 * The generating provider/model is shown per entry, not just asserted once for the whole
 * cache -- it's genuinely heterogeneous (docs/DECISIONS.md [2026-08-28]: free-tier quota
 * limits forced several provider/model switches over the course of one batch), the same
 * way every audit record already carries its own `model_versions`.
 *
...
export function AskWhyBox({ entry }: { entry: AskWhyEntry | null }) {
  ...
  return (
    <details className="rounded-md border border-border bg-surface-0 p-3 text-xs">
      ...
      <p className="mt-2 text-sm leading-relaxed text-text-secondary">{entry.text}</p>
      <p className="mt-2 text-[11px] text-text-muted">
        Generated ahead of time from the structured audit record above, by a model that
        never touched the money decision — narrated by {entry.provider}/{entry.model}.
      </p>
    </details>
  );
}
